...
def make_ERA5(ERA5_folder_uv, ERA5_folder_w, group_time_strat, group_time_end, GIIRS_ERA5_csv_path, need_reorientation=False):
    GIIRS_ERA5_csv = np.loadtxt(GIIRS_ERA5_csv_path, delimiter=',', dtype='str')

    ERA5_files_uv = sorted([
        fn for fn in os.listdir(ERA5_folder_uv)
        if fn.endswith('.nc') and (group_time_strat in fn or group_time_end in fn)
    ])

    ERA5_files_w = sorted([
        fn for fn in os.listdir(ERA5_folder_w)
        if fn.endswith('.nc') and (group_time_strat in fn or group_time_end in fn)
    ])
    ...
    for fn in ERA5_files_uv:
        print(fn)
        file_time = fn[-23:-9]
        file_path_uv = os.path.join(ERA5_folder_uv, fn)

        with xr.open_dataset(file_path_uv) as ds:
            u = np.array(ds['u'])  # (37,721,1440)
            v = np.array(ds['v'])

    for fn in ERA5_files_w:
        print(fn)
        file_time = fn[-23:-9]
        file_path_w = os.path.join(ERA5_folder_w, fn)

        with xr.open_dataset(file_path_w) as ds:
            w = np.array(ds['w'])  # (37,721,1440)

        era5_uvw = np.full((N, 37, 3), -999, dtype=np.float32)
        ...
        if np.any(valid_rc):
            ...
            w_flat = w.reshape(37, -1)

            u4 = np.take(u_flat, idx4, axis=1)  # (37,Nv,4)
            v4 = np.take(v_flat, idx4, axis=1)
            W4 = np.take(w_flat, idx4, axis=1)

            w4b = w4[None, :, :]
            u_interp = np.sum(u4 * w4b, axis=2).T
            v_interp = np.sum(v4 * w4b, axis=2).T
            w_interp = np.sum(W4 * w4b, axis=2).T

            era5_uvw[valid_rc, :, 0] = u_interp
            era5_uvw[valid_rc, :, 1] = v_interp
            era5_uvw[valid_rc, :, 2] = w_interp

        L, UVW = 37, 3
        x = era5_uvw.reshape(n_tiles_r * n_tiles_c, tile_h * tile_w, L, UVW)
        if need_reorientation:
            x = x[:, ::-1, ...]
        x = x.reshape(n_tiles_r * n_tiles_c, tile_h, tile_w, L, UVW, order='F')
        x = x.reshape(n_tiles_r, n_tiles_c, tile_h, tile_w, L, UVW)
        ERA5_grid = x.transpose(0, 2, 1, 3, 4, 5).reshape(H, W, L, UVW)
        ...
        for lev in range(L):
            ...
            if lev < len(pressure_levels):
                title_str = f"ERA5 wind vectors ({pressure_levels[lev]} hPa)_{file_time}"
                out_png = os.path.join(FIG_DIR, f"ERA5_wind_{pressure_levels[lev]}hPa_{file_time}.png")
            else:
                title_str = f"ERA5 wind vectors (lev={lev})_{file_time}"
...
def save_15min_npz_split(
    ...
    out_with = os.path.join(out_root, "with_era5_uvw")
...
if __name__ == '__main__':
    ...
    ERA5_root_uv = r'/media/ub/SU710/data/era5_uv'
    ERA5_root_w = r'/media/ub/SU710/data/era5_w'
    ...
    for fname in sorted(os.listdir(GIIRS_AGRI_dir)):
        ...
        test_path1 = os.path.join(out_root, "with_era5_uvw", f"{sample_time}.npz")
        ...
        ERA5_folder_uv = os.path.join(ERA5_root_uv, date_str)
        ERA5_folder_w = os.path.join(ERA5_root_w, date_str)
        ...
        ERA5_grid_all, ERA5_file_times = make_ERA5(
            ERA5_folder_uv, ERA5_folder_w,group_time_start, group_time_end, GIIRS_ERA5_csv_path, reorientation
        )
